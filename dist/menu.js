...
export const main = new Menu('menu-menu')
    .submenu(SchoolType.SoANS, 'school', (ctx) => (ctx.session.school_type = SchoolType.SoANS))
    .row()
    .submenu(SchoolType.SoCEA, 'school', (ctx) => (ctx.session.school_type = SchoolType.SoCEA))
    .row()
    .submenu(SchoolType.SoEEC, 'school', (ctx) => (ctx.session.school_type = SchoolType.SoEEC))
    .row()
    .submenu(SchoolType.SoMCME, 'school', (ctx) => (ctx.session.school_type = SchoolType.SoMCME))
    .row();
// First SubMenu to Pick A School's Department
const school = new Menu('school');
function sendDepartmentBySchoolType(range, start, end) {
    Object.values(DepartmentType)
        ...
        range
            ...
            ctx.editMessageText(`Please pick the Academic year for ${ctx.session.department_type} Department`);
        })
            .row();
    });
}
...
const department = new Menu('department');
function sendYearBySchoolType(range, start, end) {
...
year
    ...
    .row();
year
    .submenu('Second Semester', 'semester', (ctx) => (ctx.session.semester_type = SemesterType.sem2))
    .row();
// Back button to return to main menu
year.back('🔝main menu');
// Back button to return to Department Submenu
year.text('🔙back', (ctx) => ctx.menu.nav('department'));
const semester = new Menu('semester');
function makeCourseButtons(courses, range) {
    courses.forEach((course) => {
        range
            .text(course[0], (ctx) => {
            if (course.length < 3) {
                ctx.reply(`Sorry, But I don't have ${course[0]}'s Course Outline yet`);
            }
            else {
...
semester.dynamic((ctx) => {
    const range = new MenuRange();
    const sem = ctx.session.semester_type;
    const school = Object.keys(SchoolType)[Object.values(SchoolType).indexOf(ctx.session.school_type)];
    const year = Object.keys(YearType)[Object.values(YearType).indexOf(ctx.session.year_type)];
    const department = Object.keys(DepartmentType)[Object.values(DepartmentType).indexOf(ctx.session.department_type)];
    if (ctx.session.year_type == YearType.First) {
        ...
        makeCourseButtons(course[school][department][year][sem], range);
    }
    return range;
});
